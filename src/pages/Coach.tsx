...
import { CoachAssistantWidget } from '@/components/CoachAssistantWidget';
import { LocalAIErrorBoundary } from '@/components/LocalAIErrorBoundary';
...
export default function Coach() {
  ...
  const [, setSuggestedMealPlan] = useState<MealPlan | null>(null);
  const [, setAssistantActions] = useState<AssistantActionSuggestion[]>([]);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="space-y-6">
        {/* Coach Chat (never allowed to crash the route). Provider selection lives in Settings. */}
        <LocalAIErrorBoundary
          fallback={(
            <div className="card border border-orange-200 bg-orange-50">
              <div className="flex items-start gap-3">
                <AlertCircle className="mt-0.5 h-5 w-5 text-orange-700" />
                <div>
                  <div className="font-medium text-orange-900">Coach chat failed to render</div>
                  <div className="mt-1 text-sm text-orange-800">
                    No worries: the offline workout generator still works below. You can revisit Settings to reconfigure AI.
                  </div>
                  <div className="mt-3">
                    <a className="btn btn-secondary btn-sm" href="#/settings">Open Settings</a>
                  </div>
                </div>
              </div>
            </div>
        ...
        >
          <CoachAssistantWidget
            profile={profile}
            onSuggestedWorkoutPlan={(plan) => setGeneratedPlan(plan)}
            onSuggestedMealPlan={(plan) => setSuggestedMealPlan(plan)}
            onActions={(actions) => setAssistantActions(actions)}
          />
        </LocalAIErrorBoundary>
      ...
      {/* Profile Summary */}
        ...
        <div className="card">
          ...
          <p className="text-sm text-gray-500 mb-6">
            The built-in coach engine always works offline. AI suggestions (chat) are optional.
          </p>
